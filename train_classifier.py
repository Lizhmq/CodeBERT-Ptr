...
from transformers import (WEIGHTS_NAME, AdamW, get_linear_schedule_with_warmup, DataCollatorForLanguageModeling,
...
def load_and_cache_examples(args, tokenizer, evaluate=False):
    dataset = ClassifierDataset(tokenizer, args, logger, file_type='dev' if evaluate else 'train', block_size=args.block_size)
    return dataset         
...
def train(args, train_dataset, model, tokenizer, fh, pool):
    ...
    for idx in range(args.start_epoch, int(args.num_train_epochs)): 
        for step, batch in enumerate(train_dataloader):
            ...
            model.train()
            outputs = model._run_batch(inputs, mask=None)
            loss = model._loss(outputs, labels)
            ...
            tr_loss += loss.item() / inputs.shape[0]
            ...
            if (step + 1) % args.gradient_accumulation_steps == 0:
                ...
                if args.local_rank in [-1, 0] and args.save_steps > 0 and global_step % args.save_steps == 0:
                    ...
                    if args.evaluate_during_training:  # Only evaluate when single GPU otherwise metrics may not average well
                        ...
                        output_dir = os.path.join(args.output_dir, '{}-{}-{}'.format(checkpoint_prefix, global_step, round(results['cross_entropy'],4)))
                    else:
...
def evaluate(args, model, tokenizer, prefix="", eval_when_training=False):
    ...
    ave_loss = ave_loss / num_sample

    result = {
        "cross_entropy": float(ave_loss)
    }
...
def main():
    ...
    if pretrained:
        ...
        model = codebert(pretrained, args.device)
        model.model.resize_token_embeddings(len(tokenizer))
    # else:
    #     tokenizer = tokenizer_class.from_pretrained(args.tokenizer_dir)
    ...
    #     print(name, token, tokenizer.convert_tokens_to_ids(token))
